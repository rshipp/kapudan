...
class Widget(QtGui.QWidget, Screen):
    ...
    def on_buttonClam_clicked(self):
        ...
        command = "kfmclient openURL http://www.chakra-linux.org/wiki/index.php/Anti-Malware#ClamAV"
        self.procSettings.start(command)

    def on_buttonTomoyo_clicked(self):
        self.procSettings = QProcess()
        command = "kfmclient openURL http://www.chakra-linux.org/wiki/index.php/Using_tomoyo-tools_for_system_security"
        self.procSettings.start(command)

    def on_buttonKwallet_clicked(self):
        self.procSettings = QProcess()
        command = "kfmclient openURL http://www.chakra-linux.org/wiki/index.php/KDE_Wallet_Manager"
        self.procSettings.start(command)

    def on_buttonRootkit_clicked(self):
        self.procSettings = QProcess()
        command = "kfmclient openURL http://www.chakra-linux.org/wiki/index.php/Anti-Malware#chkrootkit_and_rkhunter"
        self.procSettings.start(command)

    def on_buttonTiger_clicked(self):
        self.procSettings = QProcess()
        command = "kfmclient openURL http://www.nongnu.org/tiger/"
        self.procSettings.start(command)
